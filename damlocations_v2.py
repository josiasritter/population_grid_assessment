...
def damlocations_plot(gdf_in, gdf_countries, yearcolors):
    ...
    ax = gdf_countries.to_crs('ESRI:54030').plot(ax=ax, color='lightgrey', edgecolor='white', linewidth=.2)
    ...
    ax = centroids.plot(column="refyear",ax=ax,markersize="size",c=centroids["colour"],legend=True,)

    # need to add existing legend back
    leg1 = ax.get_legend()

    # some bins to indicate size in legend
    _, bins = pd.cut(centroids["size"], bins=3, precision=0, retbins=True)

    # create second legend
    import matplotlib.lines as mlines
    ax.add_artist(ax.legend(handles=[mlines.Line2D([],[],color="black",lw=0,marker="o",markersize=np.sqrt(b),label=str(int(b)),)for i, b in enumerate(bins)],loc=4,))
